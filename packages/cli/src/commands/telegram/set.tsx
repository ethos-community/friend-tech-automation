...
import { setTelegramAPiKey } from '../../clients/keychain.js';
...
export default function Set(): ReactElement {
  ...
  if (!success) {
    return (
      <>
        <Text>Paste Telegram API key</Text>
        <Newline />
        <PasswordInput
          placeholder="  🔒 Enter API key"
          onSubmit={(value) => {
            try {
              setTelegramAPiKey(value)
                .then(() => {
                  setSuccess(true);
                })
                .catch((err) => {
  ...
  return success ? (
    <StatusMessage variant="success">Successfully set Telegram API key</StatusMessage>
  ) : (
